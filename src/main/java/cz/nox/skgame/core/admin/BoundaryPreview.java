...
public class BoundaryPreview {
    ...
    private static final double UNIT_STEP = 0.25;
    ...
    public BoundaryPreview(Player admin, Location p1, @Nullable Location p2, Plugin plugin) {
        this.admin = admin;
        this.p1 = p1.clone();
    ...
    private void render() {
        ...
        spawnUnitCube(p1, DUST_GREEN);

        if (p2 == null) return;
        if (!world.equals(p2.getWorld())) return;

        // Red unit cube at p2
        spawnUnitCube(p2, DUST_RED);
    ...
    private void spawnUnitCube(Location corner, Particle.DustOptions dust) {
        double x0 = corner.getBlockX(), y0 = corner.getBlockY(), z0 = corner.getBlockZ();
        double x1 = x0 + 1, y1 = y0 + 1, z1 = z0 + 1;
        spawnEdgeX(x0, x1, y0, z0, UNIT_STEP, dust);
        spawnEdgeX(x0, x1, y0, z1, UNIT_STEP, dust);
        spawnEdgeX(x0, x1, y1, z0, UNIT_STEP, dust);
        spawnEdgeX(x0, x1, y1, z1, UNIT_STEP, dust);
        spawnEdgeZ(z0, z1, x0, y0, UNIT_STEP, dust);
        spawnEdgeZ(z0, z1, x0, y1, UNIT_STEP, dust);
        spawnEdgeZ(z0, z1, x1, y0, UNIT_STEP, dust);
        spawnEdgeZ(z0, z1, x1, y1, UNIT_STEP, dust);
        spawnEdgeY(y0, y1, x0, z0, UNIT_STEP, dust);
        spawnEdgeY(y0, y1, x1, z0, UNIT_STEP, dust);
        spawnEdgeY(y0, y1, x0, z1, UNIT_STEP, dust);
        spawnEdgeY(y0, y1, x1, z1, UNIT_STEP, dust);
    }
