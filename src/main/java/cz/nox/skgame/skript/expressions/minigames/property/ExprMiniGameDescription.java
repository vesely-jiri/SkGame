...
@Description({"The description of a minigame.", "", "Supports: GET / SET / RESET."})
@Examples({"broadcast description of event-minigame"})
@Since("1.0.0")
public class ExprMiniGameDescription extends SimpleExpression<String> {
    private Expression<Object> expr;
    static { Skript.registerExpression(ExprMiniGameDescription.class, String.class, ExpressionType.COMBINED,
            "minigame description of %minigame%", "%minigame%'s minigame description"); }
    ...
    @Override protected @Nullable String[] get(Event ev) {
        ...
        Object v = mg.getValue("description"); return v == null ? null : new String[]{v.toString()}; }
    ...
    @Override public void change(Event ev, @Nullable Object[] delta, Changer.ChangeMode mode) {
        ...
        switch (mode) {
            case SET -> { if (delta == null || delta[0] == null) return; mg.setValue("description", delta[0].toString()); }
            case RESET, DELETE -> mg.removeValue("description"); } }
    @Override public boolean isSingle() { return true; }
    @Override public Class<String> getReturnType() { return String.class; }
    @Override public String toString(@Nullable Event ev, boolean d) { return "description of " + expr.toString(ev, d); }
}
